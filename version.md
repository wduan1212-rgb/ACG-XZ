...
## v120 - 2026-07-28（统一工作区本地试做，未部署）

### 本版范围

- 主平台改为单一白色工作区外壳：左侧按当前功能切换上下文，品牌区保持“星阵 + 灰色功能名”同一行；移除固定黑色功能栏、旧版界面入口和重复业务筛选，发布入口统一改名为“发布清单”。主平台缓存暂定为 `20260728-v120-shell-10`。
...
- 本轮 BGM / 接口审计继续通过主服务完整回归 `378/378`、视频工坊自身回归 `93/93`、工作区与趋势 Node 回归 `35/35`；覆盖参考图视觉识别与使用回执、图片 / 视频 API 路由、视频成片音轨合成、发布封面 3:4 生成、原子发布 / 撤回、成员隔离和代理安全。
- 所有本轮修改的主平台 JavaScript 通过 `node --check`；无限画布 TypeScript 检查和 `build:embed` 通过，最终静态文件已同步至独立 v120 工作树。
- curl 自动检查确认：`8788` 主平台命中 `20260728-v120-shell-10`，视频工坊命中 `20260727-v120-shell-6`，无限画布返回“正在打开画布”且不含旧欢迎首页；独立 `8787` 仍命中 v119 的 `20260727-v119-4`，`8765` sidecar 与 `8787` 主服务健康。未进行手动浏览器验收。

### 数据与部署

- 本版只在分支 `codex/v120-unified-workspace-shell` 的独立工作树和本机 `8788` 预览完成；尚未推送、尚未部署生产，也没有发送服务器更新消息。
- 未修改后端接口契约、数据库内容、存储结构或生产配置；仅修正共享 BGM / 剪辑图片的只读识别与成员可见性规则，未迁移、删除、取消、重跑或覆盖任何任务、草稿、账号、资产、发布数据、画布项目或视频运行数据。
- `8788` 只提供 v120 前端静态文件，业务 API 继续透明读取本机 `8787` 的 v119 基线。后续只有在用户完成手动验收并明确授权后，才可另行制定精确部署清单。
