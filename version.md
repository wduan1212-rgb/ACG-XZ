...
## v119 - 2026-07-27（客户端入口、发布分布与批量成片封面，本地待发布）
...
- 主平台本地缓存升级为 `20260727-v119-4`。本版已完成本地验收，待受保护生产部署。
...
- 本版只允许从干净目标提交精确同步代码、静态文件和版本化安装包；不得覆盖服务器 SQLite、账号 / 成员、资产 / 发布清单、uploads / composed、画布项目 / blob、视频工坊 runtime、认证缓存或私密环境。生产部署结果须在受保护部署完成后另行补录。
