...
## v120 - 2026-07-27（统一工作区本地试做，未部署）

### 本版范围

- 主平台改为单一白色工作区外壳：左侧按当前功能切换上下文，品牌区保持“星阵 + 灰色功能名”同一行；移除固定黑色功能栏、旧版界面入口和重复业务筛选，发布入口统一改名为“发布清单”。主平台缓存暂定为 `20260727-v120-shell-8`。
- 批量生产到单号工坊改用唯一的路由模块身份，进入微调后同时保留流程上一步节点和“返回批量生产”；批量会话与内部内容区继续可滚动但不显示滚动条，删除会被菜单遮挡的“本会话”标题。
- 图文工坊的统一参考图增加清楚的悬停、拖入提示和低强度动效；资产拖入补齐空 MIME / `application/octet-stream` 的真实 File、Blob、上传、缓存与导出链路，拖放反馈限制在当前工作区视口内。
...
- 语音生成把真实音色库迁入统一左栏，保留定制 / 收藏 / 系统音色、声线 / 语言筛选、音色名称和列表操作；右侧音频预览区拉高并增加轻量等待动效，收藏 / 复制和声线参数固定在右栏底部。
...
- 必跑回归全部通过：Node 工作区 / 趋势测试 `33/33`、无限画布 `34/34`、视频工坊集成 `15/15`、数字工作流与 MIME 链路 `29/29`、语音布局 `7/7`。
- 额外模块身份、客户端分发、视频运行时、成员隔离、定制创作、子应用部署、发布筛选和 8788 代理回归 `56/56` 通过；合计自动用例 `174/174`。
- 所有本轮修改的主平台 JavaScript 通过 `node --check`；无限画布 TypeScript 检查和 `build:embed` 通过，最终静态文件已同步至独立 v120 工作树。
- curl 自动检查确认：`8788` 主平台命中 `20260727-v120-shell-8`，视频工坊命中 `20260727-v120-shell-6`，无限画布返回“正在打开画布”且不含旧欢迎首页；独立 `8787` 仍命中 v119 的 `20260727-v119-4`。未进行手动浏览器验收。
