...
- 主平台改为单一白色工作区外壳：左侧按当前功能切换上下文，品牌区保持“星阵 + 灰色功能名”同一行；移除固定黑色功能栏、旧版界面入口和重复业务筛选，发布入口统一改名为“发布清单”。主平台缓存暂定为 `20260728-v120-shell-9`。
...
- curl 自动检查确认：`8788` 主平台命中 `20260728-v120-shell-9`，视频工坊命中 `20260727-v120-shell-6`，无限画布返回“正在打开画布”且不含旧欢迎首页；独立 `8787` 仍命中 v119 的 `20260727-v119-4`，`8765` sidecar 与 `8787` 主服务健康。未进行手动浏览器验收。

### 数据与部署

- 本版只在分支 `codex/v120-unified-workspace-shell` 的独立工作树和本机 `8788` 预览完成；尚未推送、尚未部署生产，也没有发送服务器更新消息。
- 未修改后端接口、数据库、存储结构或生产配置；未迁移、删除、取消、重跑或覆盖任何任务、草稿、账号、资产、发布数据、画布项目或视频运行数据。
