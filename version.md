...
## v120 - 2026-07-28（统一工作区已切换本机 8787，未部署生产）

### 本版范围

- 主平台改为单一白色工作区外壳：左侧按当前功能切换上下文，品牌区保持“星阵 + 灰色功能名”同一行；移除固定黑色功能栏、旧版界面入口和重复业务筛选，发布入口统一改名为“发布清单”。主平台缓存为 `20260728-v120-shell-13`。
- 批量生产到单号工坊改用唯一的路由模块身份，进入微调后同时保留流程上一步节点和“返回批量生产”；批量会话与内部内容区继续可滚动但不显示滚动条，删除会被菜单遮挡的“本会话”标题。
- 图文工坊的统一参考图增加清楚的悬停、拖入提示和低强度动效；资产拖入补齐空 MIME / `application/octet-stream` 的真实 File、Blob、上传、缓存与导出链路，拖放反馈限制在当前工作区视口内。整体资产的 BGM 继续只接收 MP3；剪辑素材同时接收图片和视频，写入共享剪辑标签并可跨成员显示，普通私有图片不会误进入共享库。
- 视频工坊嵌入区统一为白色并隐藏内部重复历史栏。统一左侧改为不可折叠的“历史会话”，标题旁使用小加号新建；会话默认透明，悬停才出现删除入口，并显示“已发布 N”。历史会话打开链路改为先建立 iframe 消息桥再导航，支持失败后对同一项目重试；本地 v119 视频侧车已恢复并继续读取原运行目录。成片尺寸的选中态恢复为深底白字，箭头与文字保持可读。删除仅从当前成员的 v120 本地会话列表移除，不删除服务端项目或已发布内容。视频缓存暂定为 `20260727-v120-shell-6`。
- 无限画布进入功能时直接等待并打开真实最近项目，不再渲染旧首页；最近项目、缩放控制和小地图迁入统一左栏。平台嵌入时移除画布内部标题 / 导出 / 发布顶栏及加载占位，给画布释放完整高度；嵌入式静态构建刷新为 `05swzw_vhL65PSP47qYke`。
- 语音生成把真实音色库迁入统一左栏，保留定制 / 收藏 / 系统音色和声线 / 语言筛选；声线行收紧并降低字重，每行只显示三点入口，试听、收藏、复制及有权限的编辑 / 删除统一进入可用键盘操作的下拉菜单。右侧音频预览合并为单层区域，移除空状态、生成状态和播放器的重复内框，保留轻量等待动效、状态文字和声线参数。
- 首页进一步统一白底与轻量层级：顶部删除“数据完整度”，保留三张等宽指标卡；下方发布分布占同一三列轨道的 1/3，趋势图占 2/3，分界与指标卡严格对齐。图表整行和账号表现整行等高，环图删除平台文字图例；账号表现每页展示 16 个真实账号，按 4 列 × 4 行向上下均匀铺满，悬停只保留文字与头像轻反馈，不再出现方块底色。数据助手删除四个建议问题按钮，在空白消息区改为居中的浅灰使用指引，输入框固定在底部。
- 管理员首页的“同步数据”独立创建，不再依赖“新建账号”控件是否存在，并且仍只对管理员显示。管理设置左栏拆为成员账号、产品库、模型用量、成员申请四个独立入口，每次只渲染对应内容；个人资料从管理分栏中分离，资料卡在工作区可用高度内垂直居中。
- 管理员与创作者个人菜单增加意见反馈弹窗和 `mailto:wduan1212@gmail.com`；单号账号列表、发布筛选、资产分类、顶栏折叠按钮及其与业务操作的避让继续按统一工作区收口。
- BGM 拖入链路补齐 `audio/mp3`、`audio/x-mp3` 等浏览器 MIME 别名；显式拖入 BGM 库的文件不再因原文件名含“语音”而被误排除，同一二进制进入参考音频与 BGM 时保持两个独立逻辑资产。剪辑台继续读取共享 BGM，视频工坊平台曲库继续只读主平台资产，并把选中的曲目和音量实际传入最终合成。

### 自动验证

- 必跑回归全部通过：Node 工作区 / 趋势测试 `35/35`、无限画布 `34/34`、视频工坊集成 `15/15`、数字工作流与真实 File / Blob / MIME 链路 `30/30`、语音布局与菜单交互 `8/8`。
- 额外模块身份、客户端分发、成员资料、成员隔离、定制创作、子应用部署、发布筛选和 8788 代理回归 `63/63` 通过；合计自动用例 `185/185`。
- 本轮切换前重新通过主服务完整回归 `380/380`、视频工坊自身回归 `93/93`、前端工作区 / 趋势 / 代理 / 供应商搜索回归 `47/47`；覆盖参考图视觉识别与使用回执、图片 / 视频 API 路由、视频成片音轨合成、发布封面 3:4 生成、原子发布 / 撤回、成员隔离和代理安全。供应商账号卡片的一条旧布局断言已同步为最终确认的“三个账号一排、单卡内部同一行”结构，没有回退正确界面。
- 所有本轮修改的主平台 JavaScript 通过 `node --check`；无限画布 TypeScript 检查和 `build:embed` 通过，最终静态文件已同步至独立 v120 工作树。
- curl 自动检查确认：本机 `8787` 与保留的 `8788` 预览均命中 `20260728-v120-shell-13`；`8787` 视频工坊返回新版白色工作区文案，无限画布返回“正在打开画布”且不含旧欢迎首页。`8787` 主服务与 `8765` sidecar 均健康，导演、视频生成、语音、合成、质检和平台 BGM 状态均为 ready。未进行手动浏览器验收。

### 数据与部署

- 本版位于分支 `codex/v120-unified-workspace-shell` 的独立工作树，已按用户授权把本机主入口 `8787` 从 v119 切换为 v120；`8788` 预览继续保留。尚未推送、尚未部署远程生产，也没有发送服务器更新消息。
- 未修改后端接口契约、数据库内容、存储结构或生产配置；仅修正共享 BGM / 剪辑图片的只读识别与成员可见性规则，未迁移、删除、取消、重跑或覆盖任何任务、草稿、账号、资产、发布数据、画布项目或视频运行数据。
- 本机切换只替换运行代码：v120 主服务继续显式读取原 v119 本地 SQLite、uploads、composed、canvas_blobs 与视频工坊 projects / outputs / uploads / 模型缓存。切换前后计数保持为业务文档 `941`、成员 `6`、视频项目 `22`、视频输出 `431`、视频上传 `66`、主上传 `433`、合成文件 `40`，没有迁移、删除或覆盖运行数据。若需本机回退，可停止当前 v120 启动器并从原工作目录重新执行 `自动化产品/start.command`；远程生产部署仍需另行明确授权。
