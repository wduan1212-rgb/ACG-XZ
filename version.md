...
- 修复视频文案分镜在推断主产品时出现 `primaryProducts is not defined`：为 `chainWorkshop.js` 补齐 `primaryProducts` 导入。
- 将入口、主模块、创作空间和文案分镜模块的缓存标识同步升级为 `20260714-v77-2`，确保普通刷新加载修复后的模块图。

### 验证结果

- 全部跟踪 JavaScript 通过 `node --check`，Python 服务端文件通过编译检查，`git diff --check` 通过。
- 线上 MiniMax-M3 `/api/llm/test` 返回成功，确认报错发生在请求语言模型之前，不是模型配置或 Key 失效。
- 真实浏览器加载 `chainWorkshop.js?v=20260714-v77-2`，执行原报错的产品推断路径后不再出现缺失变量，并继续运行到测试设置的网络中止哨兵；控制台错误为 0。

### 数据与部署

- 热修复提交 `b5e11ad` 已部署，只替换 4 个前端代码/入口文件；服务器数据库、JSON 状态、上传、成片、环境文件、认证缓存和模型配置均未修改。
- 部署后账号 80、成员 8、分析链接 21、指标快照 108、会话 51、音色 4，关键集合没有减少；验收期间其他成员产生的任务和素材继续正常增长。
- 回滚点：`pre-v77.1-20260714-155236`。回滚只恢复本次替换的前端文件，不回退生产业务数据。
