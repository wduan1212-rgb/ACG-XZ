...
test("all runtime modules share the v139 cache identity", async () => {
  const [indexHtml, mainJs] = await Promise.all([
    read("index.html"),
    read("js/main.js"),
  ]);
  assert.match(indexHtml, /20260803-v139-durable-usage-1/);
  assert.match(mainJs, /APP_BUILD_ID = "20260803-v139-durable-usage-1"/);
