...
class ModelUsageReceiptStoreTest(unittest.TestCase):
    ...
    def test_local_schema_preserves_v139_receipt_and_advances_forward_only(self):
        with sqlite3.connect(store.DB_PATH) as conn:
            ...
            member_control_ledger = conn.execute(
                "SELECT checksum,status FROM schema_migrations WHERE version=?",
        ...
        self.assertEqual(
            (store.MEMBER_CONTROL_SCHEMA_MIGRATION_CHECKSUM, "success"),
        ...
        self.assertEqual(
            (store.PRODUCTION_RECOVERY_SCHEMA_MIGRATION_CHECKSUM, "success"),
            recovery_ledger,
        )
        self.assertEqual(
            (store.MODEL_USAGE_SETTLEMENT_V2_SCHEMA_MIGRATION_CHECKSUM, "success"),
            settlement_v2_ledger,
        )
        self.assertEqual(
            (store.MEDIA_ISOLATION_SCHEMA_MIGRATION_CHECKSUM, "success"),
            media_isolation_ledger,
        )
        self.assertEqual(
            store.MEDIA_ISOLATION_SCHEMA_MIGRATION_VERSION,
