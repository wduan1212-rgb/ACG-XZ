...
class SupplierAccountManagementTests(unittest.TestCase):
    ...
    def test_supplier_dashboard_keeps_account_tools_and_activity_history_interactive(self):
        ...
        self.assertIn("--supplier-dashboard-height: auto;", supplier_css)
        self.assertIn(".supplier-dashboard-visuals { grid-template-columns: minmax(220px, 1fr) minmax(440px, 2fr); }", supplier_css)
        self.assertIn("height: auto;", supplier_css)
        self.assertIn("max-height: min(620px, calc(100dvh - 130px));", supplier_css)
