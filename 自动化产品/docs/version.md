...
- 多词英文别名在产品匹配时保持完整，不再把 `Token Plan` 拆成泛化的 `Token` 与 `Plan`，避免普通英文文案误命中。`baige` 与 `token-plan` 均由官方 seed 刷新陈旧副本，自定义产品继续原样保留。本版不新增数据库 schema、持久目录、后端生成 API、权限门禁、图片队列策略或批量图文/无限画布执行状态。
...
- AI 选题从“多账号整包成功或整包失败”改为按账号收敛：后端分块或单账号修复失败时仍返回已生成的标题、正文、来源和缺失账号列表，不再用一个 502 丢掉整批成功结果。
- 前端把已生成预览保存到当前批次 session，关闭弹窗、刷新页面或再次打开都能恢复；允许“先填入已生成的 N 个账号”，只填空白标题/正文，不覆盖用户手工修改。
...
- 批量图文生图改为主服务 owner-scoped 耐久 job：单次可原子登记最多 `48` 项，每项均可由服务器独立推进，真实上游并发统一交给 owner/surface 公平 provider 队列；浏览器只负责登记、查询和展示，关闭页面或刷新不再让未发出的图片停住。成功项、失败项和 provider 结果未知项逐项落盘，明确重试只补目标项，不覆盖成功图或自动重提未知调用。
- 无限画布继续使用自己的耐久 batch/job 契约；批量图文不复用画布、数字人或视频工坊业务状态。两条图片业务共享的是受控 provider 容量，不共享 production 状态机。
- 供应商已确认 image-2 的账号总容量为 `10` 路，但同一密钥还被另一个产品使用。主服务因此采用“最大 `IMAGE_SUBMIT_CONCURRENCY=10`、初始 `IMAGE_SUBMIT_INITIAL_CONCURRENCY=6`”的自适应公平队列：provider 返回繁忙就逐路收缩，持续成功再缓慢恢复到 10；批量图文、无限画布及其他图片入口仍只共享 provider 容量，不共享业务状态，也不增加每人硬限或拒绝创作的门禁。无私密假 provider `50` 请求峰值精确为 `10` 且后到 surface 不饥饿。首次真实 10 路直压时，另一个产品占用使 provider 在 HTTP 200 JSON 中返回 `code:1002`；旧 helper 把任意 2xx 当成功而未重试。候选现识别这种业务繁忙、逐次写独立 receipt，并在后台最多做覆盖约一个常见生成周期的 8 次安全退避，不把账号总容量竞争直接落成用户任务失败。本次不改同一画布整批的顺序 runner：它尚未持久完整请求，直接并发会在重启时同时留下多个未知付费结果。
- 旧积分/订阅/用量账本全部退出生产授权路径：记录失败只告警，不阻断生成、启动、RW、路由或发布。统计口径为图片按张、视频按个、语言/语音按次、AI 选题按条；详情按具体功能聚合，供判断视频工坊、批量生产等实际使用分布。
- 创意视频与数字人链路明确切割：新创意任务只使用纯故事板、用户参考图和 Seedance 2.5 音画同出，不生成口播或字幕，也不要求语音模型。视频工坊媒体增量登记只处理隔离基线之后可证明归属的新输出，启动与 60 秒后台收敛均不提交、轮询、重试 provider，历史 48 条隔离基线保持原样。

### 下一轮架构优化

- 批量视频封面仍有浏览器直连图片执行路径，应迁到独立耐久 job；AI 选题和看图写文案仍受同步 HTTP 生命周期约束，也应改为各自可恢复的服务端任务。
- 旧积分/订阅表和兼容 UI、sidecar 静态 billing 字段仍为历史兼容层，应逐步收口成只读展示后删除；不能重新进入授权、启动或发布判定。
- `model_usage_receipts` 仍同时承载 provider 幂等意图和统计遥测，下一轮应拆成不可变 operation receipt 与异步 analytics event，避免统计故障影响任务语义。
- 空团队租户首次登录时，浏览器仍会尝试把本地产品种子回推到共享 `products`，编辑成员被服务端正确 403 拒绝后页面仍可用，但会形成无效请求和控制台噪声。下一轮应把种子初始化完全收回服务端 bootstrap，浏览器只读取租户投影，不新增权限门禁。
- Seedance 2.5 对 `48x48` 参考图明确返回最小宽度 `300` 的 HTTP 400；应复用图片链统一参考图归一化，在提交前无损补边/缩放，不新增拒绝创作的门禁。

### 最终本地验收

- 主服务全量 `856 tests / OK / 1 approved skip`，视频工坊 sidecar `160/160`，Node `129/129`；Python compileall、变更 JavaScript 语法与 `git diff --check` 通过。
...
- 批量图文不再让语言模型一次返回整组 `shots` 大 JSON。每张图卡使用已分配的标题/正文节拍独立生成纯文本提示词，同账号两路并行、检查点串行落盘。单卡输出异常时只用已确认文案组装该卡，不再做整组 JSON 修复，也不阻塞同账号其他图卡。
- 图片生成改为每张独立结算：一张失败或 provider 结果未知时，已成功图片与稳定 operation key 继续保留，后续不同图片继续执行。production 进入“结果待确认/待补图片”而不是整条红色失败；只有用户明确点击才旋转缺失项的 operation key，不重提已成功项。其他账号已进审核时立即可审核/发布，不被待补账号卡住。
- 全部图片或交付真正完成时统一清理历史 production/item 错误，避免 `4/4` 或已交付仍显示旧失败。本版不改无限画布整批原子登记、顺序执行或全站两路图片 provider 容量。
- 异形参考图 transport 增加硬内存边界：`7×4096` / `4096×9` 这类极端长条先在最长边 `4096` 内按比例缩放，再用白边补到上游 `3:1` 边界；不裁剪原图，也不再因“先把 1px 短边放到 256px”制造超大中间画布。
...
- 无限画布把用户明确写出的生成数量作为最高优先级。类似“设计 10 张，图 1 放 Logo、图 2 放产品、图 3 放二维码”的指令仍生成 10 个独立任务，附件只作为参考图，不再因为出现“图 1/放/调整”等词被误判为“逐张编辑 4 个参考节点”。
- 单画布图片任务改为串行排队，服务器共享图片提交默认并发从 `3` 收敛为 `2`；用户要求的 10 张仍完整创建 10 个后台 job，只降低同一时刻送往上游的大图请求数，为批量图文和其他成员保留共享槽位。
- 服务器已经返回并保存 blob 后，本地 IndexedDB/项目检查点的短暂冲突不再把真实成功卡片降级成“生成失败”。再次进入项目时也会以现存 `assetUrl` 修复历史错标，恢复 `done` 状态和正常“海报 NN”名称。
- 网络抖动、浏览器中断或前台等待超时时，卡片改为“后台排队中/处理中，稍后自动更新”；后台 owner-scoped job 继续执行，页面自动轮询并回填，不再把可恢复的后台状态描述成终止故障。
- 团队创作者可以查看和回复同一团队其他成员交付的发布沟通；访问仍先通过不可变 resource scope，跨团队继续返回 403。百度搜索已经成功、文案模型只因 JSON 围栏/引号/转义等格式错误而失败时，只对原返回做一次严格语法修复，不重搜资料、不重跑已完整账号，也不猜写新事实。
- release/cache identity 更新为 `20260812-v1428-canvas-batch-stability-1`。本版不新增 schema、数据迁移、依赖、持久目录、权限或 Nginx 变更；部署只能同步验签代码/静态与非密钥并发参数，不覆盖生产 SQLite、账号、媒体、任务、认证或 provider 私密配置。
...
- 修复无限画布与批量图文的多参考图适配：保持原 `custom-imagemodel-gt` 模型，不切换模型。生产对照定位到旧代码在 `{image_url: ...}` 对象数组外附加了该自定义端点不支持的 `input_fidelity`；单图时上游宽松容忍，多图时立即返回 `Code 1001`。本版保留原生多图对象数组，只移除非法字段；同一模型、同一批四张生产参考图已真实返回 HTTP 200、`usedRefs=4`，不组版、不丢图。参考图 HTTP 窗口从 `180s` 对齐到后台五分钟任务内的 `270s`。原图、服务器素材、画布 Blob 和账号数据均不改写。
