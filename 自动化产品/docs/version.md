...
## v135 - 2026-08-03（本地发布候选归档与生产迁移架构方案，功能快照 `61346a4`，未推送、未部署）

### 本版范围

- 将 v123–v134 累积完成并已自动验证的本地功能整理为可审计发布候选，不再继续叠加未记录改动；功能基线仍以 v134 的静态视频、终态计费、社区资产、团队鉴权和无限画布可靠性为准。静态视频继续独立于 Seedance，按语义及存在的口播自然拆分，只要求整体时长大致合理，不建立精确时长约束，也不强制用户提供或生成口播。
...
- 工作树按“确认提交、精确忽略、可重建生成物、归属未知保留”四类收口。禁止 reset、clean、全量 stash、整目录覆盖、`git add .` 或为追求空状态而删除未知遗留。
- 旧部署 README 中的示例不能作为生产事实；部署时只使用当前受保护部署指南和按提交生成的 release manifest，不采用删除式同步。

### 审计与验证

- 本地代码审计确认 `server/main.py` 与 `server/store.py` 均接近 9,000 行，当前约 134 个主服务路由；启动阶段仍有隐式 schema/数据修复，且尚无正式 `schema_migrations` 与 `/api/ready`，因此这些项目列为生产升级前 P0 门禁，而不是在本次版本归档中冒险重写。
- 公网主入口与健康接口只读探针返回成功，但健康响应不包含发布版本、数据库或迁移状态；SSH 非交互只读探测未取得内部状态，因此不能把生产内部版本、SQLite、sidecar 或团队映射描述为已实时确认。
- 本轮重新执行的自动验证已通过：主平台 Node/UI 回归 `93/93`，主服务 Python 回归 `476/476`（另有 `8` 项按测试标记跳过），视频工坊 Python 回归 `120/120`，无限画布 TypeScript 类型检查与嵌入构建通过且静态导出 `10/10` 页面，改动 JavaScript 语法检查 `72/72`，Python 编译检查与 `git diff --check` 通过。主服务回归仅排除了 `test_client_distribution.py`：当前工作树中的历史 DMG 删除归属未确认且明确不进入暂存；目标提交仍保留 HEAD 中已跟踪的安装包，不能为让测试变绿而恢复或提交该未知删除。
...
- v123–v135 功能快照已形成本地提交 `61346a4`；本记录由随后的文档提交固化。本轮不推送、不部署，不修改生产数据库、服务器文件、环境变量、账号、媒体或私密配置。
- `.codex-lark-auth/**`、`自动化产品/.gitignore`、Project Memory 删除、旧安装包/旧 favicon/两份旧方案删除、三个 `*-chroma.png` 中间产物、运行数据、本地媒体、私密配置以及混杂的 `自动化产品/vendor/infinite-canvas/**` 哈希不进入功能快照。根目录四份交接/问题/部署/版本文档与 `自动化产品/docs/` 对应文件成对迁移；`自动化产品/design-qa.md` 的确认删除进入快照。无限画布静态闭包必须在已提交源码的干净临时检出中重建、生成哈希 manifest 后另行提交。
- 生产迁移仍等待用户明确授权。部署日必须重新取得线上 commit、缓存标识、systemd/worker、SQLite、团队/供应商映射和受保护目录清单，不能只凭健康接口执行升级。
