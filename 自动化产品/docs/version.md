...
- 修复无限画布与批量图文的多参考图请求协议：保持原 `custom-imagemodel-gt` 模型，不切换模型。短边不足 `256px` 的图片继续在 provider 临时副本上等比放大，超窄 Logo 只增加最小白色留白；多张引用不再合成参考板，而是按 TokenHub 混元生图协议要求，以 URL/data-URL 字符串数组传输，同时移除该路径不支持的 `input_fidelity`。参考图 HTTP 窗口从 `180s` 对齐到后台五分钟任务内的 `270s`。原图、服务器素材、画布 Blob 和账号数据均不改写。
