...
## v136 - 2026-08-03（本地代码收口待提交：社区幂等、首页媒体与静态视频分镜）

### 本版范围

- 当前 v136 代码候选建立在 `0e9d144` 之上，功能提交号：**待提交**。本轮未推送、未部署，未连接或修改生产服务器、数据库、账号、媒体或私密配置。
...
- 本地主平台 `8787` 与视频 sidecar `8765` 均以当前源码启动并返回 200 健康响应，检查后已停止。未执行浏览器视觉验收、真实社区发布、图片/语音/Seedance 付费生成、Git 推送或部署；功能提交将在精确暂存后形成。
