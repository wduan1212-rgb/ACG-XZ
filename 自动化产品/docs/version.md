...
- 修复无限画布与批量图文的多参考图适配：保持原 `custom-imagemodel-gt` 模型，不切换模型。生产实测确认该自定义实例只接受单张 `{image_url: ...}` 对象输入，两张/四张对象数组及新版字符串数组均立即返回 `Code 1001`。因此多张原图只在 provider 内存 transport 中无裁切、无拉伸地排入一张 `1000×1000` 普通 PNG，以经实测能进入当前模型的单图协议提交；回执仍记录原始逻辑参考图数。参考图 HTTP 窗口从 `180s` 对齐到后台五分钟任务内的 `270s`。原图、服务器素材、画布 Blob 和账号数据均不改写。
