...
- 修复无限画布多参考图中的超窄 Logo 条仍被图片上游以参数无效拒绝：短边不足 `256px` 继续等比放大，宽高比超过 `3:1` 的图片只在送往 provider 的临时副本上增加最小白色留白；多张引用再无裁切、无拉伸地组成一张白底参考板，以适配当前 MaaS 单参考输入边界，回执仍精确记录实际包含的全部逻辑原图。参考图 HTTP 窗口从 `180s` 对齐到后台五分钟任务内的 `270s`。原图、服务器素材、画布 Blob 和账号数据均不改写。
