...
import { autoAssemble, setStage, isVideoWorkshop } from "../domain/productions.js?v=20260810-v1412-publish-quota-baige-canvas-1";
...
import { toast, openVideoPreview } from "../ui/components.js?v=20260810-v1412-publish-quota-baige-canvas-1";
...
export function renderCutPage(root, p) {
  ...
  $("#cutNext", root).addEventListener("click", async event => {
    ...
    if (p.mode === "视频" && !p.artifacts?.boards?.cover?.assetId) {
      toast("未检测到封面，正在自动生成");
      try {
        const { ensureVideoCover } = await import("./chainWorkshop.js?v=20260810-v1412-publish-quota-baige-canvas-1");
