...
import { autoAssemble, setStage, isVideoWorkshop } from "../domain/productions.js?v=20260809-v141-content-governance-2";
...
import { toast, openVideoPreview } from "../ui/components.js?v=20260809-v141-content-governance-2";
...
export function renderCutPage(root, p) {
  ...
  $("#cutNext", root).addEventListener("click", async event => {
    ...
    if (p.mode === "视频" && !p.artifacts?.boards?.cover?.assetId) {
      toast("未检测到封面，正在自动生成");
      try {
        const { ensureVideoCover } = await import("./chainWorkshop.js?v=20260809-v141-content-governance-2");
