...
export async function mountCustomCanvas(
  ...
  const mountCanvasFrame = () => {
    ...
    iframe.src = `/XZ-Design/?embed=1&v=20260804-v140-usage-settlement-1${projectHash(currentProjectId)}`;
  ...
  const openProject = projectId => {
    ...
    if (!iframeReady) {
      iframe.src = `/XZ-Design/?embed=1&v=20260804-v140-usage-settlement-1${nextHash}`;
      return true;
    }
    try {
