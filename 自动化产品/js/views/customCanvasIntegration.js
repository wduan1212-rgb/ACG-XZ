...
export async function mountCustomCanvas(
  ...
  const mountCanvasFrame = () => {
    ...
    iframe.src = `/XZ-Design/?embed=1&v=20260810-v141-dashboard-metrics-1${projectHash(currentProjectId)}`;
  ...
  const openProject = projectId => {
    ...
    if (!iframeReady) {
      iframe.src = `/XZ-Design/?embed=1&v=20260810-v141-dashboard-metrics-1${nextHash}`;
      return true;
    }
    try {
