...
function isBatchPlanText(text) {
  const s = text.trim();
  if (/(建|创建|新增).{0,8}(账号|号)/.test(s)) return false;
  return /(批量|量产|创作|做|出|生成|写|安排|挑|选择|选|找|匹配|给我).{0,24}([0-9两一二三四五六七八九十]+|一些|几个|几|一批|若干)?.{0,8}(图文|笔记|素材视频|素材号|真人|数字人|视频|账号|号)|([0-9两一二三四五六七八九十]+)\s*(篇|张|条|支).{0,8}(图文|笔记|视频)/.test(s);
}
...
export function parseGoalFallback(goal) {
  ...
  const cnM = goal.match(/([0-9]+|[两一二三四五六七八九十]+)\s*(个|条|只|家|篇|张|支)\s*(账号|号|图文|笔记|视频|素材|真人|数字人)?/);
